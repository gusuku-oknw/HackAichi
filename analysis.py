...
import json
from chatgpt_analysis import analyze_image_with_gpt, analyze_cafes
# .env ファイルから環境変数を読み込む
load_dotenv('.env')
...
# カフェ情報を取得して保存
def fetch_cafe_data(save_directory='data', filename='cafes_data.json'):
    # 名駅周辺のカフェを検索
    cafes = search_cafes(nagoya_station, radius, GOOGLE_MAPS_API_KEY)
    cafe_data_list = []
    ...
    for i, cafe in enumerate(cafes[:num_cafes]):
        ...
        cafe_data = {
            ...
            "storeName": cafe_name,
            "location": {
                "latitude": location.get('lat', nagoya_station['latitude']),
                "longitude": location.get('lng', nagoya_station['longitude'])
            },
...
if __name__ == "__main__":
    ...
    fetch_cafe_data(save_directory, filename)
